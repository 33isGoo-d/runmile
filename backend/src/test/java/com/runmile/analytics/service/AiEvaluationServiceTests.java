...
class AiEvaluationServiceTests {
    ...
    void 평가_지표가_없으면_빈_응답을_반환한다() {
        ...
        when(modelRepository.findAllById(AiEvaluationService.REQUIRED_METRIC_NAMES))
                .thenReturn(List.of());
    ...
    void 모델과_효과_평가_지표를_시나리오_순서로_반환한다() {
        ...
        when(modelRepository.findAllById(AiEvaluationService.REQUIRED_METRIC_NAMES)).thenReturn(List.of(
                rmse,
                mae,
                mape
        ));
