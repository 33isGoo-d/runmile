...
export default function AdminPage() {
  const [stage, setStage] = useState<Stage>("WHERE"); const [scenario, setScenario] = useState<Scenario>("MEDIUM");
  const [overview, setOverview] = useState<AnalyticsOverview | null>(null); const [districts, setDistricts] = useState<DistrictAnalytics[]>([]);
  const [categories, setCategories] = useState<CategoryAnalytics[]>([]); const [effects, setEffects] = useState<PolicyEffect[]>([]);
  const [insights, setInsights] = useState<Insight[]>([]); const [evaluation, setEvaluation] = useState<AiEvaluation>({ baseline: null, effects: [] });
  const [loading, setLoading] = useState(true); const [notReady, setNotReady] = useState(false);
  const load = async () => { setLoading(true); setNotReady(false); try { const q=`?scenario=${scenario}`; const [o,d,c,e,i,v]=await Promise.all([getApi<AnalyticsOverview>(`/admin/analytics/overview${q}`),getApi<DistrictAnalytics[]>(`/admin/analytics/districts${q}`),getApi<CategoryAnalytics[]>("/admin/analytics/categories"),getApi<PolicyEffect[]>(`/admin/analytics/effects${q}`),getApi<Insight[]>(`/admin/analytics/insights${q}`),getApi<AiEvaluation>("/admin/analytics/evaluation")]); setOverview(o);setDistricts(d);setCategories(c);setEffects(e);setInsights(i);setEvaluation(v); } catch { setNotReady(true); } finally { setLoading(false); } };
  useEffect(()=>{void load();},[scenario]);
